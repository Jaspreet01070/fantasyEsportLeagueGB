...
const synchronizeRateLimiters = async (rateLimiters, rateLimits, methodCounts) => {
    ...
    return Promise.all(rateLimiters.map(async (limiter, index) => {
        ...
        const rateLimitsLeftFromRiot = (newRateLimits.reservoirRefreshAmount || 0) -
            (newRateLimits.reservoir || 0);
        const newReservoir = Math.min(currentReservoir, rateLimitsLeftFromRiot - requestsInFlight);
